...
export class UserRewards extends Vue {
  ...
  get validatorTxs() {
    return this.stakingTxs.filter((tx) =>
      ["AddValidatorTx", "AddPermissionlessValidatorTx"].includes(tx.txType)
    );
  }

  get delegatorTxs() {
    return this.stakingTxs.filter((tx) =>
      ["AddDelegatorTx", "AddPermissionlessDelegatorTx"].includes(tx.txType)
    );
  }
