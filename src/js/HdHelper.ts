...
const SCAN_SIZE = 100; // the total number of utxos to look at initially to calculate last index
...
class HdHelper {
  ...
  async findAvailableIndexNode(start = 0): Promise<number> {
    const addrs: string[] = [];
  ...
  getAddressForIndex(index: number): string {
    ...
    const pkHex = key.publicKey ? key.publicKey.toString("hex") : "";
